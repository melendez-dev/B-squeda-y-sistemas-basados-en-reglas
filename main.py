...
rutas = [
    ...
    ("Calle 45", "Av. Jiménez", 6, [0,1,2,3,4,5,6]),
    ("Av. Jiménez", "Zona Industrial", 7, [0,1,2,3,4,5]),
    ("Zona Industrial", "Banderas", 9, [0,1,2,3,4,5]),
    ("Banderas", "Portal Américas", 6, [0,1,2,3,4,5,6]),
    ...
    ("Av. Jiménez", "Av. 26", 3, [0,1,2,3,4,5,6]),
    ...
    ("Av. Jiménez", "Av. 63", 6, [0,1,2,3,4,5,6]),
...
coordenadas = {
    ...
    "Av. Jiménez": (-74.0733, 4.6097),
    "Zona Industrial": (-74.0958, 4.6291),
    "Av. 63": (-74.0667, 4.6489),
    "Av. 68": (-74.1114, 4.6413),
    "Banderas": (-74.1287, 4.6214),
    "Portal Américas": (-74.1572, 4.6167),
}

# Crear grafo con tráfico aleatorio
def crear_grafo(dia):
    G = nx.DiGraph()
    for u, v, tiempo, dias in rutas:
        if dia in dias:
            # Simulamos tráfico con +/- 30% variación
...
def encontrar_k_rutas(G, origen, destino, k=3):
    try:
        rutas = list(islice(nx.shortest_simple_paths(G, origen, destino, weight="weight"), k))
        tiempos = []
        for ruta in rutas:
...
def visualizar_rutas(G, rutas, coordenadas):
    ...
    for i, (path, tiempo) in enumerate(rutas[::-1]):  # invertimos para que rojo quede arriba
...
dia_str = input("Día de la semana (lunes, martes, miércoles, jueves, viernes, sábado, domingo): ").strip().lower()
